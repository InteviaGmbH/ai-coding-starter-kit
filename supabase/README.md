...
- `migrations/20260729150000_candidate_document_uploaded_at.sql` (PROJ-20 QA fix, BUG-10 — the spec requires an upload date next to the CV download link, which nothing tracked. Adds `candidates.cv_uploaded_at`; deliberately a dedicated column rather than reusing `updated_date`, which is bumped by any field change, not just a document replace.)
- `migrations/20260730090000_personnel_requests_required_workload.sql` (PROJ-14 — adds optional `personnel_requests.required_workload_percent` (0–100), the target value the candidate matching score's Pensum factor compares a candidate's own `max_workload_percent` against. No behavior change if left unset.)
